...
func TryGetEnvironmentVariable(name string) (bool, string) {
	if result, ok := _environmentVariables[name]; ok {
		return true, result
	}

	logger := log.Default()
	logger.Printf("no configuration value has been found for '%s'", name)
	return false, ""
}
